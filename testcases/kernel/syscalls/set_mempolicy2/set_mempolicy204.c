...
/*
 * We are testing set_mempolicy2() with MPOL_INTERLEAVE on mmaped buffers backed
 * by files.
 *
...
static void verify_set_mempolicy2(void)
{
	...
	TEST(set_mempolicy2(MPOL_INTERLEAVE, 0, bm->maskp, bm->size+1, NULL));

	if (TST_RET) {
		tst_res(TFAIL | TTERRNO,
		        "set_mempolicy2(MPOL_INTERLEAVE)");
		return;
	}

	tst_res(TPASS, "set_mempolicy2(MPOL_INTERLEAVE)");
...
static struct tst_test test = {
	.setup = setup,
	.cleanup = cleanup,
	.test_all = verify_set_mempolicy2,
	.forks_child = 1,
	.needs_root = 1,
	.all_filesystems = 1,
	.mntpoint = MNTPOINT,
