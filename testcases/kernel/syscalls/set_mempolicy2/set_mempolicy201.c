...
/*
 * We are testing set_mempolicy2() with MPOL_BIND and MPOL_PREFERRED.
 *
 * For each node with memory we set its bit in nodemask with set_mempolicy2()
...
static void setup(void)
{
	...
	/*
	 * In most cases, set_mempolicy201 finish quickly, but when the platform
...
static void verify_mempolicy(unsigned int node, uint16_t mode)
{
	...
	TEST(set_mempolicy2(mode, 0, bm->maskp, bm->size+1, NULL));

	if (TST_RET) {
		tst_res(TFAIL | TTERRNO,
		        "set_mempolicy2(%s) node %u",
		        tst_mempolicy_mode_name(mode), node);
		return;
	}

	tst_res(TPASS, "set_mempolicy2(%s) node %u",
	        tst_mempolicy_mode_name(mode), node);
...
{
	unsigned int i;
	uint16_t mode = n ? MPOL_PREFERRED : MPOL_BIND;
